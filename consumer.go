...
func (c *Consumer) MarkPartitionOffsetReset(topic string, partition int32, offset int64, metadata string) {
	c.subs.Fetch(topic, partition).MarkOffsetReset(offset+1, metadata)
}
...
func (c *Consumer) FetchOffsetsManual(subs map[string][]int32) (map[string]map[int32]offsetInfo, error) {
	return c.fetchOffsets(subs)
}
...
func (c *Consumer) CommitOffsetsWithRetryManual(retries int) error {
	err := c.CommitOffsets()
